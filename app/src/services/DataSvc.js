...
const DataService = {
  ...
  getData(count, allCountries = false) {
    const countries = allCountries ? _allCountries : _someCountries
    const data = []
  ...
  // Pie Data Source
  getPieData() {
    const names = ['オレンジ', 'リンゴ', '梨', 'バナナ', 'パイナップル']
    const data = []
  ...
  getSimpleDataSet() {
    const dtOct = new Date(2015, 9, 1)
    const dtDec = new Date(2015, 11, 1)
    const data = [
      {
        product: 'Wijmo',
        country: 'アメリカ',
  ...
  getMultiRowData() {
    ...
    for (let i = 0; i < 50; i++) {
      const first = firstNames[DataService.randBetween(0, firstNames.length - 1)]
      const last = lastNames[DataService.randBetween(0, lastNames.length - 1)]
      customers.push({
        id: i,
        name: `${first} ${last}`,
    ...
    for (let i = 0; i < 20; i++) {
      const shipped = wjcCore.DateTime.addDays(today, -DataService.randBetween(1, 3000))
      orders.push({
        id: i,
  ...
  getLayoutDefs() {
    ...
    const cityMap = new wjCGrid.DataMap(_cities)
    const ldOneLine = [
